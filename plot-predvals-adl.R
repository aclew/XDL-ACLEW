...
# Conditional
X.cond <- model.matrix(
  lme4::nobars(formula(ads.rand.st.zinb)[-2]), newdata)
beta.cond <- fixef(ads.rand.st.zinb)$cond
pred.cond <- X.cond %*% beta.cond
# Zero-inflation
ziformula <- ads.rand.st.zinb$modelInfo$allForm$ziformula
X.zi <- model.matrix(lme4::nobars(ziformula), newdata)
beta.zi <- fixef(ads.rand.st.zinb)$zi
...
pred.condpar.psim <- mvrnorm(
  1000, mu = beta.cond, Sigma = vcov(ads.rand.st.zinb)$cond)
pred.cond.psim <- X.cond %*% t(pred.condpar.psim)
# Zero-inflation
pred.zipar.psim <- mvrnorm(
  1000, mu = beta.zi, Sigma = vcov(ads.rand.st.zinb)$zi)
...
quantity.rand.st$ads_mph.st.rnd <- round(
  quantity.rand.st$ads_mph.st, 0)
real.count <- plyr::ddply(quantity.rand.st,
  ~ tchiyr.std + SpkrType +
    nsk.std + group_corpNE,
  summarize,
  m = median(ads_mph.st.rnd),
  mu = mean(ads_mph.st.rnd))
...
real.count.AllSpkrs <- real.count %>%
  ...
  summarize(
    mean.m = mean(m),
    mean.mu = mean(mu))
ads.model.AllSpkrs.plot <- ggplot(
